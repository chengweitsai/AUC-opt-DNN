...
mnist_train = mnist.train.images - np.stack(
    [mnist_train_mean for _ in range(train_num)])
...
mnist_test = mnist.test.images - np.stack(
    [mnist_test_mean for _ in range(test_num)])
...
with graph.as_default():
    ...
    # axes=[1]:  indicates to 'clip_by_norm' for each row of model.w
