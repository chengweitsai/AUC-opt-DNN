...
class AUCModel(object):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('feature_extraction'):
            ...
            self.bn_conv0 = tf.contrib.layers.batch_norm(self.h_relu0, 
                                                         center=True, scale=True, 
                                                         scope='bn0')

            self.W_conv1 = tf.get_variable("W_conv1",[5,5,4,16],dtype=tf.float32,initializer=tf.random_normal_initializer(0.0,1e-3))
            ...
            self.feature_ave = tf.Variable(tf.zeros([batch_size,7*7*16],dtype=tf.float32),name='feature_ave')
...
def train_and_evaluate(training_mode, graph, model, verbose=True):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for i in range(num_steps):
            ...
            lr = 2e-5
            wc = wc + lr
            batch_loss, frac, A, B, Alpha,inner_product,  _ = \
                               sess.run([model.loss, fraction, model.a, model.b, model.alpha, \
                                         model.inner_prod, train_op],
        ...
        inner_product = sess.run([model.inner_prod],
...
for i in range(FLAGS.num_epochs):
    ...
    fopen.close()
fopen =open('./output/'+FLAGS.output_file,'a')
fopen.write('testing data average overall AUC score over '+str(FLAGS.num_epochs)+' epochs: '+str(auc_ave)+'\n')
fopen.close()
